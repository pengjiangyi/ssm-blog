...
<![endif]-->
		<title>主页消息推送管理</title>
	</head>

	<body>
		<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 主页消息推送管理<span class="c-gray en">&gt;</span> 信息管理
		...
		<div class="page-container">
			...
			<div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a href="javascript:;" onclick="information_add('添加信息','${request.contextPath}/admin/information-add.html','','310')" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 添加信息</a></span> <span class="r">共有数据：<strong>${information?size}</strong> 条</span> </div>
			<div class="mt-20">
				<table class="table table-border table-bordered table-hover table-bg table-sort">
					...
					<#list information as item>
						<tr class="text-c">
							...
							<td><input type="checkbox" value="${item.id}" name="informationbox"></td>
							<td>${item.id}</td>
							<td>${item.text}</td>
							...
							<td class="td-manage">
								...
								<a title="编辑" href="javascript:;" onclick="information_edit('编辑','${request.contextPath}/admin/information-update.html/${item.id}','','','310')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6df;</i></a>
								</a>
								<a title="删除" href="javascript:;" onclick="information_del(this,${item.id})" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
							</td>
...
<script type="text/javascript" src="${request.contextPath}/resources/assets/hui-admin/static/h-ui.admin/js/H-ui.admin.js"></script> <!--/_footer 作为公共模版分离出去-->
	...
	var rootPath = getRootPath()
			...
			function datadel() {
				var arr = new Array();
				$.each($('input[name=informationbox]:checked'), function() {
					arr.push($(this).val());
				});
				if(arr.length == 0) {
					layer.alert("请先选中");
				...
				$.ajax({
					type: "post",
					url: rootPath+"/deleteInformation",
					traditional: true,
					data: {
						list:arr
					},
			...
			/*信息-添加*/
			function information_add(title, url, w, h) {
			...
			/*信息-编辑*/
			function information_edit(title, url, id, w, h) {
				//传id，到后台查出相应的数据后返回
			...
			function information_del(obj, id) {
				layer.confirm('确认要删除吗？', function(index) {
					$.ajax({
						type: 'POST',
						url: rootPath+'/deleteInformationById',
						data:{
						id:id
						},
