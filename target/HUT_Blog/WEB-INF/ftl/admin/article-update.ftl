...
<article class="page-container">
			...
			<div class="row cl">
					<label class="form-label col-xs-2 col-md-2"><span class="c-red">*</span>文章id：</label>
					<div class="formControls col-xs-10 col-sm-9">
						<input type="text" class="input-text" value="${article.id}" placeholder="在这里输入文章标题" id="id" name="id">
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-xs-2 col-md-2"><span class="c-red">*</span>文章标题：</label>
					<div class="formControls col-xs-10 col-sm-9">
						<input type="text" class="input-text" value="${article.title}" placeholder="在这里输入文章标题" id="title" name="title">
					</div>
				</div>
		<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>文章简介：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<textarea name="content" id="content" cols="" rows="" class="textarea" placeholder="说点什么...最少输入10个字符">${article.content}</textarea>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-10 col-sm-9">
						<textarea id="container" name="html" type="text/plain" >${article.html}</textarea>
					</div>
				</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-10 col-sm-9">
						<input type="text" class="input-text" value="${article.author}" placeholder="输入文章作者" id="author" name="author">
					</div>
				</div>
      ...
      <option value="${item.id}" >${item.name}</option>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9 skin-minimal">
						...
						<input style="width: 90%;" class="input-text" type="text" id="picture" name="cover" value="${article.imgurl}"/><input type="button" onclick="upImage();" class="btn btn-primary radius" value="上传"></input>
					</div>
				</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-10 col-sm-9">
						<input type="text" class="input-text" value="${article.keywords}" placeholder="输入网站关键字,多个关键字用逗号隔开" id="keywords" name="keywords">
					</div>
				</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-10 col-sm-9">
						<input type="text" class="input-text" value="${article.des}" placeholder="输入网站描述" id="des" name="des">
					</div>
				</div>
...
var tagid=${article.tag.id}
var numbers=$("#tag").find("option");
for (var j = 1; j < numbers.length; j++) {
if ($(numbers[j]).val() == tagid) {
